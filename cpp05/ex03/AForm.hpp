...
/*   Updated: 2025/02/02 16:30:19 by maiboyer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define FGradeExceptionImpl(NAME, TY)                                          \
  AForm::GradeToo##NAME::GradeToo##NAME() {                                    \
    this->message = "Form Grade is too " TY ": no reason given";               \
  }                                                                            \
                                                                               \
  AForm::GradeToo##NAME::~GradeToo##NAME() throw(){};                          \
                                                                               \
  AForm::GradeToo##NAME::GradeToo##NAME(int grade, std::string reason) {       \
    std::stringstream ss;                                                      \
    ss << "Form Grade is too " TY ": " << grade << ";reason: " << reason;      \
    this->message = ss.str();                                                  \
  }                                                                            \
                                                                               \
  const char *AForm::GradeToo##NAME::what() const throw() {                    \
...
public:
  ...
  // this does the checks
  void execute(const Bureaucrat &by) const;
  // this does the actual action
  virtual void action(const Bureaucrat &by) const = 0;
...
std::ostream &operator<<(std::ostream &os, AForm const &tc);
